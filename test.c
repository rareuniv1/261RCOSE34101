...
// 프로세스 초기화 함수
void Create_Process(Process job_pool[], int num_processes, int active_io_devices) {
    for (int i = 0; i < num_processes; i++) {
        ...
        for (int j = 0; j < p->IO_Req_Num; j++) {
            ...
            p->IO_Reqs[j].IO_Device = (rand() % active_io_devices) + 1;      // [1, 5]
...
void Process_IO_Ticks(WaitQueue queues[], ReadyQueue* rq, int system_time, int active_io_devices) {
    for (int i = 1; i <= active_io_devices; i++) {
        if (queues[i].count > 0) {
...
AlgoResult Execute_Simulation(Process origin_pool[], int num_processes, AlgoType algo, int time_quantum, int show_logs, int active_io_devices) {
    ...
    // Ready queue + Wait Queue초기화
    Init_ReadyQueue(&ready_queue);
    for (int i = 1; i <= active_io_devices; i++) {Init_WaitQueue(&device_wait_queues[i]);}
    // 시스템 타이머 초기화
    ...
    while (terminated_count < num_processes && system_time < MAX_TIME) {
        ...
        Process_IO_Ticks(device_wait_queues, &ready_queue, system_time, active_io_devices);
    ...
    while (1) {
        ...
        switch (menu_choice) {
            case 1:
                ...
                active_io_devices = (rand() % MAX_IO_DEVICES) + 1; // 디바이스 개수 : [1,5]
                Create_Process(job_pool_origin, num_processes, active_io_devices); // 프로세스 생성
                printf("\n=================================================================\n");
            ...
            case 2: case 3: case 4: case 6: case 7: // 프로세스들 생성하기
                Execute_Simulation(job_pool_origin, num_processes, (AlgoType)menu_choice, 0, 1, active_io_devices);
                break;
            ...
            case 5:
                ...
                if (scanf("%d", &time_quantum) != 1) {
                    while (getchar() != '\n');
                    time_quantum = (rand() % 20) + 1;
                    printf("Invalid input. Random Time Quantum selected: %d\n", time_quantum);
                }
                else if (time_quantum <= 0) {
                    time_quantum = (rand() % 20) + 1;
                    printf("Invalid input. Random Time Quantum selected: %d\n", time_quantum);
                }
                Execute_Simulation(job_pool_origin, num_processes, ALGO_RR, time_quantum, 1, active_io_devices);   // 그 다음에 실행 ㄱㄱ
                break;
            ...
            case 8:
                ...
                for (int a = 2; a <= 7; a++) {
                    benchmark_results[a] = Execute_Simulation(job_pool_origin, num_processes, (AlgoType)a, time_quantum, 0, active_io_devices);
                }
                printf("\n=================================================================\n");
            ...
            case 9:
                ...
                for (int test = 1; test <= num_test_sets; test++) {
                    ...
                    int current_io_devices = (rand() % MAX_IO_DEVICES) + 1;
                    Process temp_pool[MAX_PROCESSES];
                    Create_Process(temp_pool, current_num, current_io_devices);

                    for (int a = 2; a <= 7; a++) {
                        AlgoResult res = Execute_Simulation(temp_pool, current_num, (AlgoType)a, time_quantum, 0,current_io_devices);
